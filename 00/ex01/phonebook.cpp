...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/11 18:31:17 by huahmad           #+#    #+#             */
/*   Updated: 2026/01/17 13:27:34 by huahmad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Phonebook::Phonebook( void ) { // constructor that initializes index and max
    this->index = 0;
    this->max = 8;
	return;
}

Phonebook::~Phonebook( void ) { // destructor obv
	return;
}
...
int Phonebook::checklist ( int index, std::string intput_str ) {
    if (std::cin.eof())
        return (2);
    else if (intput_str.length() > 1 || !std::isdigit(intput_str[0]))
    {
        system("clear");
        std::cout << "+——————————————————————————————————————+" << std::endl;
